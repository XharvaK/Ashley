...
from the then-live state forward (2026-08-23 conversion basis). That basis
is historical; it does not describe present state.
...
[Cross-Phase Architecture](Ashley_Cross_Phase_Architecture.md#8-current-fact-documentation-policy).
The [Routing Status](../Routing_Status.md) is the routing ownership and
resolution guide (with a frozen historical audit appendix, never current
bindings). Schema, capability, channel, and model facts should normally be read
...
## 3. Historical live-state snapshot (2026-08-27; not present state)

This section used to resolve volatile programme facts inline. That role is
closed: this roadmap no longer answers what is currently implemented,
deployed, promoted, routed, or owner-selected. Present state is resolved per
the fact-kind rules in [`AGENTS.md`](../../AGENTS.md) §3 from source,
exact-candidate evidence, or production observation (`UNKNOWN` when
unobservable). Do not refresh values in this file.

What this section evidenced during the 2026-08 documentation campaign:

- Production observation and the exact-candidate C1–C5
  production-acceptance record
  (`COGNITIVE_MATURATION_C1_C5_PRODUCTION_ACCEPTANCE.md`)
  established the C1–C5 base capabilities as implemented, production
  deployed, and production accepted under the observe ceiling — each
  unpromoted and non-live. The C1 Qualification Bootstrap stood as an
  implemented, independently reviewed source candidate, not deployed, with
  no qualification campaign started.
- Exact-candidate Sandbox evidence closed the V2 M1–M7 delivery gate, with
  M7 limited to the named `patch_export` profile.
- A source-state survey recorded then-current ownership distinctions:
  Discord-only channel, `nuclear.db` / `continuity.db` ownership,
  capability gates, and direct unprivileged Bubblewrap as the V2 path.

The exact SHAs, maturity rows, and mismatch notes behind that survey were
point-in-time values. They are preserved in Git history and in the bound
evidence packets — not here.
...
## 5. Engineering milestones from the then-live state

Milestones below convert already-named roadmap items into dependency-ordered
engineering work as of the 2026-08-23 conversion. They are bounded intent,
not a live progress tracker: which milestone is current delivery is an
owner decision per task, resolved live, not a sentence in this file. They do not add architecture. Closing evidence is the
...
- **Owner-selected delivery:** Sandbox → Model Fabric → Operational Continuity.
  During the 2026-08 campaign the Sandbox leg closed through named M7
  `patch_export` and the owner selected Model Fabric as the next mechanism
  track. Whether that selection is still current is an owner decision per
  task, resolved live — not a sentence in this file.
...
Sandbox was the prior owner-selected delivery. Exact-candidate evidence
closed M1–M7 at `48bad019fe601d5c871a54dd9902879862c6e96a` during the
2026-08 campaign, with M7 limited to `patch_export`. Model Fabric was then
the owner-selected next mechanism track. Its first **code** milestone was
**MF-M1**, not historical F1-obs.
...
| G1 | Close M4 production acceptance | Mechanism | G0 | **Closed:** exact-candidate packet `M4_PRODUCTION_ACCEPTANCE_553553b0d0ee.md` (filename identity; not in this `e36613b` integration tree). In-tree generic `M4_PRODUCTION_ACCEPTANCE.md` remains | Treating local or physical evidence alone as acceptance |
...
During the 2026-08 campaign, M5, M6, and the named M7 `patch_export` profile
were production accepted at the exact candidate above. Their rows preserve
architectural ordering and scope; they were never the current implementation
backlog. F1/MF-M1 was then the owner-selected next **code** cut, locally
implemented at `d918572c`. Acceptance and production promotion remain
separate claims, resolved from exact-candidate evidence — not from these
rows.
...
During the 2026-08 campaign the C1–C5 base capabilities were implemented,
production deployed, and production accepted under the observe ceiling at the
exact production candidate named in the register below. Each capability was
individually `observe`, unpromoted, and non-live. `IMPLEMENTED` / `DEPLOYED` /
`ACCEPTED` is not `PROMOTED` / `LIVE`. The separate C1 Qualification Bootstrap
source candidate was implemented and independently reviewed, but was not
deployed and had not started a C1 qualification campaign. Exact evidence:
`COGNITIVE_MATURATION_C1_C5_PRODUCTION_ACCEPTANCE.md`.
The rows below remain architectural closing-evidence definitions. They do not
authorize promotion, and they do not report present maturity.
...
Luna execution additionally requires the owner baseline gate defined by that
...
promotion dashboard. The Status column records the 2026-08 campaign reading
in frozen index vocabulary (`CURRENT PHASE CONTRACT` names the contract
role, not a live delivery claim). It is not maintained: resolve present
maturity from exact-candidate evidence, source, or production observation.

Sandbox Autonomy is a closed dated gate. During the 2026-08 campaign the
owner-selected mechanism delivery was Model Fabric; whether that selection
is still current is owner-decided per task.

During the same campaign the C1–C5 base capabilities (Memory / Evidence,
Context Budget, Learned Autonomy, Cognitive Graduation, Relational
Graduation) were implemented, production deployed, and production accepted
under the observe ceiling. Each was `observe`, unpromoted, and non-live.
Deployed / accepted is not promoted / live. The separate C1 Qualification
Bootstrap was an implemented, independently reviewed source candidate and
was not deployed.
...
Sandbox. The Sandbox delivery gate closed through named M7 `patch_export`
during the 2026-08 campaign, when the owner selected Model Fabric as the
next mechanism track. Current-facing ownership is
...
Worker entry now lives in [`AGENTS.md`](../../AGENTS.md), which routes each
task to only the material it triggers. The path below is preserved for
provenance; do not treat it as a mandatory reading ladder.

1. Read [`VISION.md`](../../VISION.md) and governing documents in authority
   order.
2. Read this roadmap. Use §3 as the bounded 2026-08-27 historical snapshot
   (never as live state), §4 for work classes, and §5 for
   engineering milestones from the then-live state forward. §6 is phase contracts,
...
5. Read current source, exact-candidate evidence, or production observation
   for volatile facts, per the fact-kind rules in AGENTS. Never a status
   dashboard.
