...
export function thoughtOutputCompatibilityInstruction(): string {
  ...
  return [
    ...
    "On a later turn that revisits an occupied concern, use its supplied dimensions.status and dimensions.reliability as uncertainty context; uncertaintyDisplay is Thought-authored presentation, not a Host inference. Budget exhaustion is operational evidence and never a semantic conclusion.",
    ...
    'Semantic class binding: semanticClass:"observation" requires observation_intent; semanticClass:"effect" requires effect_intent. readOnly describes whether the governed operation mutates its bound project or candidate; readOnly does not convert an effect-class operation into an observation. project.inspect is the bounded Thought-adopted project inspection objective and its request selects exactly one concrete M2 read surface: project.read_file, project.list_directory, or project.search_text. Those concrete project surfaces use observation_intent. workspace.verify is project_verification governed recipe execution and uses effect_intent even when read-only.',
    "A bounded inquiry experiment may pair Thought-named M3 workspace evidence steps with a recipe-only M4 workspace.verify effect step under one objective and current budget. The operator recipe allowlist is default-deny, and a failed verification receipt is evidence for Thought rather than a verdict on Ashley. This inquiry path admits neither changeset.author nor patch_export. For a separate proposal path, Thought may adopt one bounded candidate objective in an Owner-private conversation, author it in a separately admitted candidate workspace, require a matching successful M4 receipt, adjudicate acceptance, and emit patch_export with adjudication:\"accept\". patch_export is a retained, witnessed proposal artifact only: it never applies, commits, pushes, deploys, or chooses Owner notification. Notification is a separate Thought-authored publication/disclosure effect and may be omitted while the proposal remains retained.",
