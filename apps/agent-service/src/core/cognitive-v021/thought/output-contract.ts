...
export function thoughtOutputCompatibilityInstruction(): string {
  ...
  return [
    ...
    "Thought authors concern and occupancy deltas. Set occupancy only for explicitly authored or supplied concerns; match; no Host backfill. resolved and quarantined concerns are not eligible for occupied projection.",
    "Capability reality is host-owned input: operationCapabilities identify available operations, their canonical family, readOnly and requiresProject properties, observation/effect class, request fields, operator-bound fields, and authorized project IDs. Use only available operations and authorized IDs; operation metadata never selects an operation. semanticObservations carries the non-Sandbox internal read concern.inspect: Owner-private-only, single dormant concernRef, no project or operator binding. concern.inspect request is exactly {concernRef}.",
