...
export function thoughtOutputCompatibilityInstruction(): string {
  ...
  return [
    ...
    "Capability reality is host-owned input: operationCapabilities identify available operations, their label, bounded description, input/output/evidence contracts, authority conditions, hard limits, uncertainty, canonical family, readOnly and requiresProject properties, observation/effect class, request fields, operator-bound fields, and authorized project IDs. Use only available operations and authorized IDs; operation metadata never selects an operation. semanticObservations carries the non-Sandbox internal read concern.inspect: Owner-private-only, no project or operator binding. concern.inspect request is exactly {concernRef}, reading one concern from the bounded supplied inspectable window (dormant, resolved, quarantined, or unestablished) and returning its statement plus cognitive status and quarantine kind where known; or, only when the Host authorized discover this cycle, exactly {discover} with optional non-empty cursor and positive integer limit (default 32, max 64), paging content-free concernId/cognitiveStatus/quarantineKind items in that same window. concernRef and discover are mutually exclusive. An inspect ref or discover page is read authority only and never authorizes a write.",
    'Semantic class binding: semanticClass:"observation" requires observation_intent; semanticClass:"effect" requires effect_intent. readOnly does not convert an effect-class operation into an observation. project.inspect is the only current semantic project-read capability. Its request is route-neutral: projectId plus optional locator/question/focus/maxSteps; no direct/worker/provider/model/quota fields and no low-level primitive names. workspace.verify: effect_intent, read-only.',
