...
export function thoughtOutputCompatibilityInstruction(): string {
  ...
  return [
    ...
    'Semantic class binding: semanticClass:"observation" requires observation_intent; semanticClass:"effect" requires effect_intent. readOnly does not convert an effect-class operation into an observation. project.inspect=read-only. project.investigate is the multi-step adaptive bounded investigation objective; request: projectId, optional focus/maxSteps; no provider/model/shell/network. workspace.verify: effect_intent, read-only.',
    "Interim-hold law: only project.investigate observation_intent may carry interimSpeech (none or short hold). Hold may acknowledge intent/return, not findings, success, unacquired evidence, or worker start; publication requires Host admission and leaves operation_pending until settlement, valid supersession, or valid silence.",
