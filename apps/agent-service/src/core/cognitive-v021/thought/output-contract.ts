...
export function thoughtOutputCompatibilityInstruction(): string {
  ...
  return [
    ...
    "Capability reality: operationCapabilities expose label/description, input/output/evidence contracts, authority, limits, uncertainty, family, readOnly/requiresProject, semantic class, request/operator fields, and authorized project IDs. Use only available operations/IDs; metadata never selects. semanticObservations carries the non-Sandbox internal read concern.inspect: Owner-private-only, no project or operator binding. concern.inspect request is exactly {concernRef}, reading one concern from the bounded supplied inspectable window (dormant, resolved, quarantined, or unestablished) and returning its statement plus cognitive status and quarantine kind where known; or, only when the Host authorized discover this cycle, exactly {discover} with optional non-empty cursor and positive integer limit (default 32, max 64), paging content-free concernId/cognitiveStatus/quarantineKind items in that same window. concernRef and discover are mutually exclusive. An inspect ref or discover page is read authority only and never authorizes a write.",
    ...
    `Registered operationKind values are syntax vocabulary: ${REGISTERED_OPERATION_KINDS.join(", ")}. Registration is not permission, availability, or an instruction to choose. The actual supported choice surface is the advertised semantic capabilities in capabilityReality.operationCapabilities and semanticObservations: select only operationKinds advertised there with available:true, and only projectId values listed in their authorizedProjectIds. Names that parse but are not advertised as available capabilities are not usable choices, and unavailable advertised capabilities cannot be dispatched.`,
